...
For a more complete example, reference the unit tests, and the source files in package github.com/ngConsulti/affinity/group package, where Affinity uses its own RBAC to control access to user-group administration.

github.com/ngConsulti/affinity/server exposes an HTTP API over the group service. The command-line interface in cmd/ is a utility to launch the service and connect to it, using Ubuntu SSO as an identity provider.
